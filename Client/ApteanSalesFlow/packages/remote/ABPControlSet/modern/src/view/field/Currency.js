...
Ext.define('ABPControlSet.view.field.Currency', {
    ...
    config: {
         ...
         *
         * Default value for arrow value increment / decrement.
         */
        stepValue: 1
    ...
    defaults: {
        currencyDisplayField: 'name',
        currencyValueField: 'abbr'
    },
    ...
    buildCombo: function (config) {
        return {
            ...
            disabled: !config.store || config.store.getCount() <= 1,
            listeners: {
    ...
    buildNumberField: function (config) {
        return {
            ...
            stepValue: config.step ? config.step : 1,
    ...
    onNumberKeyDown: function (numberField, e) {
        ...
        if (e.ctrlKey || e.keyCode === 32 || e.altKey || /^[A-Z]$/i.test(e.browserEvent.key)) {
            ...
            var currentValue = numberField.getValue(),
                minValue = numberField.getMinValue(),
                maxValue = numberField.getMaxValue(),
                stepValue = numberField.stepValue;
            if (e.keyCode === 40) {
    ...
    // Change selection on Alt + ArrowKey
